...
class B2Service:
    ...
    async def download_pdf_sample(self, b2_url: str, sample_size: int = 1024 * 1024) -> bytes:
        """
        Download a sample of PDF content for analysis.
        This is useful for document characteristic analysis without downloading the full file.
        
        Args:
            b2_url: B2 download URL with fileId parameter
            sample_size: Maximum size of sample to download (default: 1MB)
            
        Returns:
            PDF sample content as bytes
        ...
        try:
            ...
            logger.info(f"Downloading PDF sample from B2 with file ID: {file_id}")
            
            # Download file using B2 API with range request
            # Note: B2 SDK doesn't directly support range requests, so we'll use HTTP fallback
            return await self._download_sample_via_http(b2_url, sample_size)
            
        except Exception as e:
            logger.error(f"Failed to download PDF sample from B2 URL {b2_url}: {str(e)}")
            raise

    async def _download_sample_via_http(self, b2_url: str, sample_size: int) -> bytes:
        """
        Download a sample of PDF content using HTTP range request.
        
        Args:
            b2_url: B2 download URL
            sample_size: Maximum size of sample to download
            
        Returns:
            PDF sample content as bytes
        """
        try:
            logger.info(f"Downloading PDF sample via HTTP: {b2_url}")
            
            headers = {'Range': f'bytes=0-{sample_size-1}'}
            
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.get(b2_url, headers=headers)
                
                # Handle partial content response
                if response.status_code == 206:  # Partial Content
                    pdf_sample = response.content
                elif response.status_code == 200:  # Full content (smaller than requested)
                    pdf_sample = response.content[:sample_size]
                else:
                    response.raise_for_status()
                    pdf_sample = response.content[:sample_size]
                
                if not pdf_sample:
                    raise RuntimeError("Downloaded sample is empty")
                
                # Basic PDF validation - check for PDF header
                if not pdf_sample.startswith(b'%PDF'):
                    raise RuntimeError("Downloaded sample is not a valid PDF")
                
                logger.info(f"Successfully downloaded PDF sample ({len(pdf_sample)} bytes)")
                return pdf_sample
                
        except Exception as e:
            logger.error(f"Failed to download PDF sample via HTTP: {str(e)}")
            raise
